...
[Obsolete]
internal static class BelTMathOld
{
    // Таблица 2 из СТБ 34.101.31-2020    
    internal static readonly byte[] SBoxH =
    [
    ...
    {
        // Заменяем каждый байт через S-box H
        // Порядок: u & 0xFF (u1), (u >> 8) & 0xFF (u2), и т.д.
        uint substituted =
    ...
    internal static void MultiplyGF128(Span<byte> t, ReadOnlySpan<byte> r)
    {
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            for (int j = 0; j < 8; j++)
            {
                // Если бит установлен, XOR-им результат с текущим V
                if ((b & (1 << j)) != 0)
                {
                    for (int k = 0; k < 16; k++) res[k] ^= v[k];
                }

                // Умножение v на x (сдвиг всей 128-битной строки влево)
                byte carry = (byte)((v[15] & 0x80) != 0 ? 0x87 : 0x00);

                // Сдвиг 128-битного значения влево
                byte shiftCarry = 0;
                for (int k = 0; k < 16; k++)
                {
                    byte nextCarry = (byte)((v[k] & 0x80) != 0 ? 1 : 0);
                    v[k] = (byte)((v[k] << 1) | shiftCarry);
                    shiftCarry = nextCarry;
                }

                // Редукция по модулю f(x)
                v[0] ^= carry;
            }
        }
        res.CopyTo(t);
    ...
    private static void ShiftLeft(Span<byte> data)
    {
        byte carry = 0;
        for (int i = 0; i < 16; i++)
        {
            byte nextCarry = (byte)((data[i] & 0x80) != 0 ? 1 : 0);
            data[i] = (byte)((data[i] << 1) | carry);
            carry = nextCarry;
        }
    }

    // Инкремент 128-битного счетчика s = s ⊞ 1
    internal static void Increment128(Span<byte> s)
    {
        for (int i = 0; i < 16; i++)
        {
            if (++s[i] != 0) break;
