...
[Obsolete]
internal sealed class BelTBlockOld : IBelTBlockOld
{
    private readonly uint[] _roundKeys; 
    private bool _disposed;

    public BelTBlockOld(ReadOnlySpan<byte> key)
        : this()
    {
    ...
    {
        uint a = BinaryPrimitives.ReadUInt32LittleEndian(input[0..4]);
        uint b = BinaryPrimitives.ReadUInt32LittleEndian(input[4..8]);
        uint c = BinaryPrimitives.ReadUInt32LittleEndian(input[8..12]);
        uint d = BinaryPrimitives.ReadUInt32LittleEndian(input[12..16]);
        ...
        for (int i = 1; i <= 8; i++)
        {
            ...
            // 1) b = b ^ G5(a + k1)
            b ^= BelTMathOld.G(a + k1, 5);
            // 2) c = c ^ G21(d + k2)
            c ^= BelTMathOld.G(d + k2, 21);
            // 3) a = a - G13(b + k3)
            a -= BelTMathOld.G(b + k3, 13);
            // 4) e = G21(b + c + k4) ^ i
            uint e = BelTMathOld.G(b + c + k4, 21) ^ (uint)i;
            // 5) b = b + e
            b += e;
            c -= e;
            d += BelTMathOld.G(c + k5, 13);
            b ^= BelTMathOld.G(a + k6, 21);
            c ^= BelTMathOld.G(d + k7, 5);
        ...
        // Финальная запись (в конце 8-го такта слова уже стоят как b, d, a, c)
        BinaryPrimitives.WriteUInt32LittleEndian(output[0..4], b);
        BinaryPrimitives.WriteUInt32LittleEndian(output[4..8], d);
        BinaryPrimitives.WriteUInt32LittleEndian(output[8..12], a);
        BinaryPrimitives.WriteUInt32LittleEndian(output[12..16], c);
    }
